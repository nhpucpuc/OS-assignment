...
static void * ld_routine(void * args) {
...
#endif
	struct pcb_t ** procs = (struct pcb_t**)malloc(sizeof(struct pcb_t*)*num_processes);
	int i = 0;
	...
	while(sorted == 0)
	{
		...
		{
			if(ld_processes.start_time[j-1] > ld_processes.start_time[j])
			{
				sorted = 0;
				long temp = ld_processes.start_time[j-1];
				ld_processes.start_time[j-1] = ld_processes.start_time[j];
				ld_processes.start_time[j] = temp;

				long temp_prio = ld_processes.prio[j-1];
				ld_processes.prio[j-1] = ld_processes.prio[j];
				ld_processes.prio[j] = temp_prio;

				char* temp_path = ld_processes.path[j-1];
				ld_processes.path[j-1] = ld_processes.path[j];
				ld_processes.path[j] = temp_path;

				struct pcb_t* temp_pcb = procs[j-1];
				procs[j-1] = procs[j];
				procs[j] = temp_pcb;
				break;
			}
			else if(ld_processes.start_time[j-1] == ld_processes.start_time[j] && procs[j-1]->prio > procs[j]->prio)
			{
				sorted = 0;
				long temp = ld_processes.start_time[j-1];
				ld_processes.start_time[j-1] = ld_processes.start_time[j];
