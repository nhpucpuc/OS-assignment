 ...
 */
 ...
 int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
 {
   ...
   /* Get the page to MEMRAM, swap from MEMSWAP if needed */
   if (pg_getpage(mm, pgn, &fpn, caller) != 0){
    printf("pg_setval: invalid page access\n");
      return -1; /* invalid page access */
   }
 ...
 int __write(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE value)
 {
  ...
  {
    printf("__write: invalid memory identify\n");
    pthread_mutex_unlock(&mmvm_lock);
    return -1;
  }
 ...
 int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
 {
   ...
   // Duyệt qua danh sách vùng trống để tìm vùng phù hợp
   while (rgit != NULL)
   {
     ...
     if (sizeOfFreeVMReg >= size)
     {
       ...
       // Cập nhật lại vùng trống
       rgit->rg_start += size;
       if (rgit->rg_start > rgit->rg_end)
       {
         // Xóa vùng trống nếu không còn hợp lệ
